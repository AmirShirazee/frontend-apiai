...
export default function TestPage() {
  ...
  const { editorSettings, isLoading: isLoadingEditorSettings } = useUserData(session!, status);
  const [editorOptions, setEditorOptions] = useState<EditorOptions>(defaultEditorOptions);
  ...
  const { data, isLoading, isError } = useGetTestsQuery(userId!, { skip: !userId });
  ...
  return (
    ...
    >
      ...
      >
        <CodeDisplay code={testCode} darkMode={true} editorOptions={editorOptions!} theme={theme} />
      </div>
      <EditorSettingsModal
        isOpen={isModalOpen}
